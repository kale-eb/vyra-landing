...
for(const s of fs.readdirSync(root)){ if(!fs.statSync(path.join(root,s)).isDirectory())continue;
 for(const f of fs.readdirSync(path.join(root,s))){ const t=fs.readFileSync(path.join(root,s,f),"utf8").replace(/^---[\s\S]*?---/,""); files++;
  for(const [k,re] of Object.entries(rules)){ const n=(t.match(re)||[]).length; if(n){ totals[k]=(totals[k]||0)+n; per[s+"/"+f]=(per[s+"/"+f]||0)+n; } } } }
