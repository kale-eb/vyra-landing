...
> Cut a 60-second studio tour from these clips. Open on the close-up of the preserved flower jars, then the walk in from the door. Go zone by zone in the order I walked. For each zone use the walking shot and then the close-up. Keep my one line per zone and cut everything else. Add a small label caption for each zone in the top left.

> Slow down any pan that feels fast, and remove the clip where the dog walks through.

> Match the color between the window-side shots and the back-wall shots so the room looks like one light.
...
- [Vlog](/formats/vlog)
- [Art supplies review](/formats/art-supplies-review)
