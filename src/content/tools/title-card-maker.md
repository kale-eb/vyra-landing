...
category: tools
facts:
  - "Best for: Video openers, chapter breaks, recaps, before-and-after reveals, event titles"
  - "Input: The words, the moment, and a style note"
  - "Output: A timed title on the timeline, exported in MP4 up to 4K or WebM up to 1080p"
...
```
Add a title card that says "Chapter 1" at the 3-second mark for 2 seconds. Big, centered, the project's title font, no animation.
```

```
Open the video on a title that says "one week, one painting" over the wide shot of the studio. Large type behind me so my head overlaps the letters.
```

```
Make a cute vintage title card: substantial cursive lettering, soft butter yellow fill with a thin dark blue outline, small star stickers around it, transparent background.
```
